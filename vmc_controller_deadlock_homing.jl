...
const StringMsg = pyimport("std_msgs.msg").String
const PointMsg = pyimport("geometry_msgs.msg").Point
const LinkDataMsg = pyimport("vmc_interfaces.msg").LinkData
const RobotConfigMsg = pyimport("vmc_interfaces.msg").RobotConfiguration
...
const LINKS_TO_CAPTURE = [          # Links to get included in the initial configuration message
            "fr3_link1", 
            "fr3_link2", 
            "fr3_link3", 
            "fr3_link4", 
...
function ros_vm_controller(
    ...
    try
        ...
        # Initial Configuration Publisher
        config_pub = node.create_publisher(RobotConfigMsg, "/vmc/initial_link_config", 10)
        ...
        # Prepare the robot configuration message
        config_msg = RobotConfigMsg()

        links_list = []         # Temporary list to store the LinkData messages
        
        for link_name in LINKS_TO_CAPTURE
            ...
            try 
                ...
                p.y = pos[2]
                p.z = pos[3]
                link_item.position = p         # Link Position
        ...
        # Prepare the message 
        config_msg.links = pylist(links_list)

        # Send the message
        println("📤 Sending RobotConfiguration...")

        sleep(1.0)                          # Sleep to wait for everything is set up correctly

        config_pub.publish(config_msg)  

        # sleep(0.5) 

        # config_pub.publish(config_msg)      # Safety second publish
