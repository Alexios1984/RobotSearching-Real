...
const DIST_NOSE = 0.2               # Distance from camera frame to nose point along Z axis
...
state_channel = Channel{Vector{Float64}}(4)     # Buffer for joint states
...
add_component!(vms, TanhSpring("cam_to_target_error"; max_force=10.0, stiffness=500.0); id="attraction_spring") 
add_component!(vms, LinearDamper(5.0 * identity(3), "cam_to_target_error"); id="attraction_damper")
...
for i in 1:N_REPULSORS
    ...
    add_component!(vms, GaussianSpring(err_id; max_force=-15.0, width=0.05); id="repulsor_spring_$i")
end
...
for link_name in COLLISION_LINKS
    ...
    for i in 1:N_REPULSORS
        ...
        add_component!(vms, GaussianSpring(error; max_force=-15.0, width=0.05); id=sping_id)
    end
...
function f_setup(cache)
    ...
    return (target_id, repulsor_ids, cam_frame_id)
end

function f_control(cache, t, args, dt)

    (target_id, repulsor_ids, _) = args

    # Non-blocking check for new target data
    if isready(target_channel)

        # Get Data from Target & Obstacles Channel 
...
function ros_vm_controller(
    ...
    try
        ...
        println("DEBUG [1]: Publishers created.")
        ...
        sub_target = node.create_subscription(VmcControlTarget, "/vmc/target&_obstacles", target_callback_wrapper, 10)
        ...
        try
            (target_id, rep_ids, cam_frame_id) = args
...
"""
Callback for Target & Obstacles Positions Messages
"""
